...
testFoo =
    FunctionDecl "foo"
        ...
        (CompoundStmt
            [ DeclVarExpr
                (VarDeclStmt Integer "y" Equal
                    (ParmCallDeclLiteral (IntLiteral 42)))

            , IfStmt
                (BinaryOpExpr
                    (BinaryOpParm (ParmCallDeclIdent "x"))
                    Eq
                    (BinaryOpParm (ParmCallDeclLiteral (IntLiteral 0))))
                (CompoundStmt
                    [ RetStmt
                        (BinaryOpConst
                            (ParmCallDeclLiteral (IntLiteral 0)))
                    ])
                Nothing

            , RetStmt
                (BinaryOpExpr
                    (BinaryOpParm (ParmCallDeclIdent "y"))
                    Div
                    (BinaryOpParm (ParmCallDeclIdent "x")))
            ])
        (Just Integer)
...
testBar =
    FunctionDecl "bar"
        ...
        (CompoundStmt
            [ DeclVarExpr
                (VarDeclStmt Integer "y" Equal
                    (ParmCallDeclLiteral (IntLiteral 1)))
            ...
            , ForStmt
                ...
                (Just
                    (DeclAssignStmtUnary
                        (UnaryOperatorExpr "y" IdentIncrement)))
                (CompoundStmt
                    [ IfStmt
                        (BinaryOpExpr
                            (BinaryOpParm
        ...
        (CompoundStmt
            [ CallExpr
                (CallExprDecl "bar"
                    [ ParmCallDeclExpr
                        (CallExprDecl "foo"
                            [ ParmCallDeclLiteral (IntLiteral 100) ])
                    ])
            ])
        Nothing
...
main = do
    putStrLn ">>> FOO"
    putStrLn (compileDecl testFoo)

    putStrLn ">>> BAR"
    putStrLn (compileDecl testBar)

    putStrLn ">>> BAZ"
    putStrLn (compileDecl testBaz)
