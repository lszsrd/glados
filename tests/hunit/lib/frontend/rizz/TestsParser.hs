...
testIf1 = TestCase (assertEqual "parser \"(in func) if (foo(y)) {}\""
    (Right [A.FunctionDecl "f" [A.ParmVarDeclExpr A.Integer "a"] (A.CompoundStmt [A.DeclVarExpr (A.VarDeclStmt A.Integer "y" T.Equal (A.ParmCallDeclLiteral (T.IntLiteral 0))),A.IfStmt (A.BinaryOpConst (A.ParmCallDeclExpr (A.CallExprDecl "foo" [A.ParmCallDeclIdent "y"]))) (A.CompoundStmt []) Nothing]) (Just A.Integer)])
    (runParse "fn f(Int: a) -> Int { Int y = 0; if (foo(y)) {} }"))
...
testIf4 = TestCase (assertEqual "parser \"(in func) if (foo(y) == 0) {}\""
    ...
    (runParse "fn f(Int: a) -> Int { Int y = 0; if (foo(y) == 0) {} }"))
...
testFor1 = TestCase (assertEqual "parser \"(in func) for (Int a = 10;;) {}\""
    (Right [A.FunctionDecl "f" [A.ParmVarDeclExpr A.Integer "b"] (A.CompoundStmt [A.ForStmt (Just (A.VarDeclStmt A.Integer "a" T.Equal (A.ParmCallDeclLiteral (T.IntLiteral 10)))) Nothing Nothing (A.CompoundStmt [])]) (Just A.Integer)])
    (runParse "fn f(Int: b) -> Int { for (Int a = 10;;) {} }"))
testFor2 = TestCase (assertEqual "parser \"(in func) for (Int a = 10; a < 12; a++) {}\""
    ...
    (runParse "fn f(Int: b) -> Int { for (Int a = 10; a < 12; a++) {} }"))
testFor3 = TestCase (assertEqual "parser \"(in func) for (; foo == 0; a--) {}\""
    (Right [A.FunctionDecl "f" [A.ParmVarDeclExpr A.Integer "a"] (A.CompoundStmt [A.DeclVarExpr (A.VarDeclStmt A.Integer "a" T.Equal (A.ParmCallDeclLiteral (T.IntLiteral 10))),A.ForStmt Nothing (Just (A.BinaryOpExpr (A.BinaryOpParm (A.ParmCallDeclIdent "foo")) T.Eq (A.BinaryOpParm (A.ParmCallDeclLiteral (T.IntLiteral 0))))) (Just (A.DeclAssignStmtUnary (A.UnaryOperatorExpr "a" T.IdentDecrement))) (A.CompoundStmt [])]) (Just A.Integer)])
    (runParse "fn f(Int: a) -> Int { for (; a == 0; a--) {} }"))
